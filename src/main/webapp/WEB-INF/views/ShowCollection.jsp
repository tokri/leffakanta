...
<c:if test="${not empty movieList}">
    <c:if test="${searchValue == null}"><form id="search" action="searchcollection" method="POST">
        ...
        <div id="note"><p/>Search results for '${searchValue}':<p /><a class="menu" href="collection?page=1">(Show All)</a></div><p />
    </c:if>    
    ...
    <c:if test="${pageCount>1}">
        ...
        <p class="right">
            <c:if test="${page > 1}"><a class="menu" href="collection?page=${page - 1}">← Previous</a></c:if>
            <c:if test="${page <= 1}"><span class="inactive">← Previous</span></c:if>&nbsp;&nbsp;        
            <c:if test="${page < pageCount}"><a class="menu" href="collection?page=${page + 1}">Next →</a></c:if>
            <c:if test="${page >= pageCount}"><span class="inactive">Next →</span></c:if>
        </p>
    </c:if><br/><br/>
    <p class="info"><c:if test="${searchValue == null}">You have <b>${movieCount}</b> movie<c:if test="${movieCount > 1}">s in your collection.</c:if></c:if>
