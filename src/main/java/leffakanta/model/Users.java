...
public class Users {   
    ...
    public List<User> getUserList(int page, int pageSize){
        String sql = "SELECT user_id, username, is_admin FROM \"user\" ORDER BY username ASC OFFSET ? LIMIT ?";
    ...
    public List<User> searchUserList(String searchValue){
        String sql = "SELECT user_id, username, is_admin FROM \"user\" WHERE LOWER(username) LIKE ? ORDER BY username ASC";
    ...
    public int searchUserCount(String searchValue){
        String sql = "SELECT COUNT(*) FROM \"user\" WHERE LOWER(username) LIKE ?";
